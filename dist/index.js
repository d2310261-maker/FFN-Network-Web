...
if (process.env.NODE_ENV === "production") {
  import("postgres").then(({ default: postgres }) => {
    ...
    const { drizzle: drizzlePostgres } = __require("drizzle-orm/postgres-js");
    db = drizzlePostgres({ client: sql2, schema: schema_exports });
  });
} else {
  import("ws").then(({ default: ws }) => {
    const { Pool, neonConfig } = __require("@neondatabase/serverless");
    const { drizzle: drizzleServerless } = __require("drizzle-orm/neon-serverless");
    neonConfig.webSocketConstructor = ws;
    const pool = new Pool({ connectionString: process.env.DATABASE_URL });
    db = drizzleServerless({ client: pool, schema: schema_exports });
  });
}
if (!db) {
  try {
